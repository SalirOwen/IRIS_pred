...
with tab1:
    for i, s in mapping.items():
        subset = df[df['target']==s]  #篩選符合s的索引取出
        ax.scatter(subset['sepal length (cm)'],subset['sepal width (cm)'],
                   label=i, c=colors[s])
    ax.set_xlabel('sepal length (cm)')
    ax.set_ylabel('sepal width (cm)')
    ax.legend()
    st.pyplot(fig)  #streamlit的呈現畫布,與plt.show一樣
...
X_scaler = pd.DataFrame(scaler.fit_transform(X), columns=iris.feature_names)  #縮放後的資料
